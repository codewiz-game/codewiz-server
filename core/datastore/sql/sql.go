...
func NewDataStore(dbMap *gorp.DbMap) *SQLDataStore {
	ds := &SQLDataStore{DbMap: dbMap, meta: make(map[reflect.Type]metaMap)}
	return ds
}

func (ds *SQLDataStore) AddRepositoryWithName(model interface{}, name string) {
...
func (ds *SQLDataStore) Insert(record interface{}) error {
	...
	if existing != nil {
		...
		if existingStatusField.IsValid() {
			if status := existingStatusField.Interface(); status == datastore.Active {
			...
			} else if status == datastore.Deleted {
...
func (ds *SQLDataStore) Select(model interface{}, whereClause string, args ...interface{}) ([]interface{}, error) {
	...
	if statusColumn != "" {
		...
		args = append(args, datastore.Deleted)
	}
