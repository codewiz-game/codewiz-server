...
var (
	activeValue  reflect.Value = reflect.ValueOf(datastore.Active)
	deletedValue reflect.Value = reflect.ValueOf(datastore.Deleted)
)
...
type BaseRecord struct {
	IDValue      int64                `db:"ID, primarykey, autoincrement"`
	Status       datastore.StatusCode `db:"Status" db-meta:"status"`
...
func (record BaseRecord) ID() sql.NullInt64 {
	return sql.NullInt64{Int64: record.IDValue, Valid: record.IDValue != -1}
}
