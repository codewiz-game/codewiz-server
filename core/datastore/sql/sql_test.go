...
import (
	"database/sql"
	"github.com/crob1140/codewiz/core/datastore"
...
func initTestDataStore() (*SQLDataStore, error) {
	...
	ds.AddRepositoryWithName(testRecord{}, "Test")
	return ds, nil
}
...
func TestSQLDataStore_Insert_FailsOnExistingPrimaryKey(t *testing.T) {
	...
	updated.IDValue = record.ID().Int64
...
func TestSQLDataStore_Insert_PreviouslyDeletedRecord(t *testing.T) {
	...
	record := &testRecord{BaseRecord: *NewRecord(), String: "ABC", Integer: 20}
	record.Status = datastore.Deleted
...
func TestSQLDataStore_Delete_SucceedsOnExistingRecord(t *testing.T) {
	...
	if record.Status != datastore.Deleted {
		t.Errorf("Status was not marked as deleted")
	}

	// Ensure that a search for deleted records returns this record
	deleted := &testRecord{}
	err = ds.DbMap.SelectOne(deleted, "SELECT * FROM Test WHERE Status = ?", datastore.Deleted)
	if err != nil {
		t.Error(err)
	}
...
func TestSQLDataStore_Delete_FailsOnMissingRecord(t *testing.T) {
	...
	if record.Status != datastore.Transient {
